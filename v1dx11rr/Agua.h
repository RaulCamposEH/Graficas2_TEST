...
public:
	AguaRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext)
	{
		d3dContext = D3DContext;
		d3dDevice = D3DDevice;
		this->ancho = ancho;
		this->alto = alto;
		CargaParametros(texname, 100.0f);
	}
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float Movetext, XMFLOAT3 m_CamPos)
	{
		...
		D3DXMatrixTranslation( &translationMat, 0.0f, 30.0f, 6.0f );
		D3DXMATRIX ry;
		D3DXMatrixRotationY(&ry,0.01);
		viewMatrix *= ry;
    
		D3DXMATRIX worldMat = translationMat;//= rotationMat * translationMat;
